...
outNeighbors node graph = S.map (\x -> snd x) (S.filter (\x -> node == (fst x)) (edges graph))
...
inNeighbors node graph = S.map (\x -> fst x) (S.filter (\x -> node == (snd x)) (edges graph))
...
removeNode node graph = let ns = S.delete node (nodes graph)
                            es = S.filter (\x -> not ((node == (fst x)) || (node == (snd x)))) (edges graph)
                        in (ns, es)
...
splitNode old news graph = let ns = S.union (S.delete old (nodes graph)) (S.fromList news)
                               es = let oldIsVertex x = not ((old == (fst x)) || (old == (snd x)))
                                        toOld = [(x, y) | x <- news, y <- S.toList (outNeighbors old graph)]
                                        fromOld = [(x, y) | x <- S.toList (inNeighbors old graph), y <- news]
                                    in S.union (S.filter oldIsVertex (edges graph)) (S.fromList (toOld ++ fromOld))
                           in (ns, es)
...
mergeNodes :: Ord a
           ...
           -> StandardGraph a  -- graful existent
           -> StandardGraph a  -- graful obținut
mergeNodes prop node graph = let nsWithoutProp = S.filter (\x -> not (prop x)) (nodes graph)
                                 ns = if ((S.size nsWithoutProp) == (S.size (nodes graph))) then 
                                        nodes graph
                                      else S.insert node nsWithoutProp
                                 es = let outNew = S.map (\x -> snd x) (S.filter (\x -> ((prop (fst x)) && (not (prop (snd x))))) (edges graph))
                                          inNew = S.map (\x -> fst x) (S.filter (\x -> ((prop (snd x)) && (not (prop (fst x))))) (edges graph))
                                          newToNew = S.map (\x -> snd x) (S.filter (\x -> ((prop (snd x)) && ((prop (fst x))))) (edges graph))
                                          toNew = [(node, y) | y <- (S.toList outNew)]
                                          fromNew = if (S.null newToNew) then 
                                                        [(x, node) | x <- (S.toList inNew)] 
                                                    else [(x, node) | x <- (S.toList inNew)] ++ [(node, node)]
                                          notHasProp x = (not (prop (snd x))) && (not (prop (fst x)))
                                      in S.union (S.filter notHasProp (edges graph)) (S.fromList (toNew ++ fromNew))
                             in (ns, es)

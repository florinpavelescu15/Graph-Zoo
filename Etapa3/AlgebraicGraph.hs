...
outNeighbors node graph = case graph of 
                                ...
                                (Connect x y) -> if (S.member node (nodes x)) then
                                                    (S.union (outNeighbors node x) (nodes y)) 
                                                 else S.union (outNeighbors node x) (outNeighbors node y)
...
inNeighbors node graph = case graph of 
                            ...
                            (Connect x y) -> if (S.member node (nodes y)) then
                                                (S.union (inNeighbors node y) (nodes x))
...
splitNode node targets = extend (\x -> if (x == node) then fromNodes targets else (Node x))
                            where fromNodes targets = if (null targets) then 
                                                            Empty 
                                                      else Overlay (Node (head targets)) (fromNodes (tail targets))
